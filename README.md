...
v1.2 已将这套架构落成可运行的 SAP 风格财务 Demo：企业语义目录、已批准血缘关系、Schema Linking、Join Planner、Query Planner、确定性 SQL 编译、只读校验、结果验证和 Grounding 均有对应实现。DeepSeek 只参与意图理解、澄清和已验证证据选择；表关系、权限、SQL 执行和数字核对由程序与企业配置控制。

可行性由真实 `deepseek-flash` 实验支持，而不是只停留在架构图：36 道分层题、2 轮重复、3 组对照，共 216 次调用。完整 trusted 管线通过率为 **72/72（100%）**；带业务目录但没有权威血缘的 baseline 为 **88.89%**；只依赖模型和物理表名的 raw baseline 为 **81.94%**。trusted 在可答、澄清、不支持和安全边界题上均为 100%，并且每条回答都有完整 trace、血缘 ID、只读校验和独立 Gold 数值核对。

这组实验支持“企业语义 + 权威血缘 + 确定性规划/校验能够降低模型计划错误”的结论。题集使用虚拟财务账套，规模和场景仍小于生产系统，结果不能直接外推为所有 SAP 版本、企业权限环境或商业产品的生产排名。详见 [v1.2 实验报告](v1.2/eval/EXPERIMENT_REPORT.md) 和 [原始评测结果](v1.2/eval/final_report_deepseek_flash.json)。
...
flowchart TD
    ...
    R --> S[② Business Semantic<br/>企业业务语义检索]
    S --> D{指标、口径、时间和权限明确？}
    D -->|否| C
    D -->|是| M[③ Metadata Retrieval<br/>权限过滤的企业元数据检索]
    M --> L[④ Schema Linking<br/>业务概念绑定物理字段]
    L --> J[⑤ Join Graph / Planner<br/>血缘、粒度、基数和桥表]
    J --> P[⑥ Query Planner<br/>类型化只读逻辑计划]
    P --> G[⑦ 确定性 SQL 编译或受限 NL2SQL]
    G --> V[⑧ SQL Validator<br/>对象、权限、成本、只读校验]
    V -->|可修复的结构错误| G
    V -->|通过| X[⑨ 只读数据库执行]
    X --> Y[⑩ Result Validation<br/>类型、金额、质量、对账、新鲜度]
    Y --> I[⑪ Insight LLM<br/>类型化业务事实解释]
    I --> Z[⑫ Grounding<br/>数字、单位、时间和来源核验]
    Z --> O[最终回答、图表、明细、证据]
    E[(企业语义目录)] --> S
    E --> L
    E --> P
...
目标架构采用“模型理解，程序约束和执行”的分工。DeepSeek 不拥有企业事实、权限或表关系的最终决定权；凡是可以由规则、元数据、血缘和安全策略确定的内容，都由程序自动完成。下表中的“可选”表示模型可以提高交互能力，但安全边界和确定性校验不依赖模型。当前 v1.1 Demo 的自然语言问答仍要求配置 DeepSeek；下表描述 v1.2 企业架构的职责目标，不表示 v1.1 已实现全部模块。

| 阶段 | DeepSeek 参与方式 | 自动化程序与企业配置的责任 |
|---|---|---|
| ① Router | **可选**：识别问题类型、候选业务域和缺失信息 | 根据业务域白名单、能力矩阵和会话范围作最终判断 |
| ② Business Semantic | **参与**：从用户表达中提取业务词，生成澄清问题 | 只返回企业已批准的指标、同义词、公式和时间口径 |
| ③ Metadata Retrieval | **可选**：改写检索词、补充候选概念 | 按权限和版本检索企业目录；权威血缘表优先 |
| ④ Schema Linking | **可选**：提出业务词到字段的候选映射 | 校验表、字段、类型、代码值和版本是否真实存在 |
| ⑤ Join Planner | **不参与关联决策**；可解释缺失关系 | 根据批准的血缘、主外键、粒度、基数和桥表自动规划 JOIN，并阻止多对多误算 |
| ⑥ Query Planner | **参与**：提取指标、维度、筛选、时间、排序和限制 | 将意图转换为类型化逻辑计划，套用指标模板并检查可加性 |
| ⑦ SQL 编译/生成 | **受限可选**：在结构化计划约束下生成 SQL 候选 | 优先使用确定性编译器；限制方言、对象、函数和查询范围 |
| ⑧ SQL Validator | **不负责安全放行**；可根据错误提出修复候选 | 由静态规则执行语法、对象、权限、成本、只读和危险操作校验 |
| ⑨ 数据库执行 | **不参与** | 只读账号、安全视图、RLS/CLS、超时、取消和行数限制 |
| ⑩ Result Validation | **不参与数值判定**；可辅助生成异常说明 | 程序检查类型、重复累计、金额、对账、新鲜度和质量阈值 |
| ⑪ Insight | **参与**：把已验证的类型化事实组织成业务语言 | 只向模型提供已验证结果、口径和允许的解释范围 |
| ⑫ Grounding | **可参与改写**，不能新增数字或来源 | 程序逐项核对数值、单位、日期、维度、公式和证据引用 |

其中，**Join Planner 的关联路径由程序确定**；DeepSeek 只在上游识别用户提及的业务对象。**Query Planner 的意图提取可由 DeepSeek 参与，逻辑计划由程序按企业指标和查询模板生成**。企业没有提供批准的语义、字段绑定、血缘或权限时，系统必须澄清、拒答或标记“尚未配置”，不能让模型自行补全。
