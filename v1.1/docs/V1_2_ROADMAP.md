...
> 可交付的企业只读架构包已整理到仓库根目录 [`v1.2/`](../../v1.2/)，其中明确企业输入契约、权威血缘关系表、只读边界和按企业定制的语义澄清规则。

本文件是后续构建路线，**不是已经实现的能力清单**。v1.1 是可核对的小型财务 Demo；v1.2 的可交付形态是企业只读架构包，见仓库根目录 `v1.2/`。它明确企业必须提供或确认的语义、澄清、血缘、权限、质量和验收输入。

## 1. 目标与边界

第一条真实业务链继续选择财务应收与回款，先获得一个明确SAP版本、公司范围、账套和数据接口的试点。平台支持注册更多业务域，但正式接入销售、采购或库存要分别由企业提供该领域的语义包、澄清规则、血缘输入、权限策略和验收集。
...
| 业务域 | 固定应收与回款 | 领域注册机制，每个领域由企业确认自己的口径和边界 |
| 业务目录 | 本地JSON | 可版本化、审批、回滚的指标与实体目录 |
| 元数据 | 固定小目录 | 企业权威血缘/目录输入 + 授权过滤、混合检索、重排与桥接关系补全 |
